...
def histNetwork(df, min_citations=0, sep=";", network=True):
    ...
    # Accept both original case (Scopus, Web_of_Science) and ETL uppercase (SCOPUS, WEB_OF_SCIENCE)
    db_upper = str(db).upper()
    if db_upper in ("WEB_OF_SCIENCE", "WOS", "WEB OF SCIENCE"):
    ...
    elif db_upper in ("DIMENSIONS", "PUBMED", "COCHRANE", "THE_LENS"):
        try:
            results = scopus(M, min_citations=min_citations, sep=sep, network=network)
        except Exception:
...
def scopus(M, min_citations=0, sep=";", network=True):
    ...
    M_merge['PP'] = M_merge.apply(
        lambda row: f"{row['BP']}-{row['EP']}" if pd.notna(row['BP']) else np.nan, axis=1)
    ...
    if network:
        ...
        WLCR = WLCR.pivot_table(
            index='SR_citing', columns='SR_cited', values='value', fill_value=0)
