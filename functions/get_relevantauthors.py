from www.services import *


def get_relevant_authors(df, num_of_authors, frequency='n_docs'):
    data = df.get()
    data = data.dropna(subset=['AU'])
    data['AU'] = data['AU'].apply(lambda x: x if isinstance(x, list) else [])

    all_authors = [author for sublist in data['AU'] for author in sublist]
    author_counts = pd.Series(all_authors).value_counts()

    freq_label = 'N. of Documents'

    if frequency == 'percentage':
        freq_label = 'Percentage'
        author_counts = (author_counts / len(data) * 100).round(1)
    elif frequency == 'freq_measure':
        freq_label = 'Fractionalized Frequency'
        fractional_counts = data['AU'].apply(
            lambda authors: 1 / len(authors) if authors else 0)
        fractional_authors = [
            (author, fractional_counts.iloc[i])
            for i, authors in enumerate(data['AU'])
            for author in authors
        ]
        fractional_df = pd.DataFrame(fractional_authors, columns=['Author', 'Weight'])
        author_counts = (fractional_df.groupby('Author')['Weight']
                         .sum().sort_values(ascending=False).round(1))

    author_counts = author_counts.reset_index()
    author_counts.columns = ['Authors', freq_label]
    author_counts['Authors'] = author_counts['Authors'].str[:50]
    table_relevant_authors = author_counts.copy()

    if num_of_authors > len(author_counts):
        num_of_authors = len(author_counts)
    author_counts = author_counts.head(num_of_authors)

    max_x = float(author_counts[freq_label].max()) if len(author_counts) > 0 else 1

    fig = go.Figure()

    for i, row in author_counts.iterrows():
        fig.add_shape(type='line', x0=0, x1=row[freq_label], y0=i, y1=i,
                      line=dict(color='#e0e0e0', width=5), layer='below')

    fig.add_trace(go.Scatter(
        x=author_counts[freq_label],
        y=list(range(len(author_counts))),
        mode='markers+text',
        marker=dict(
            size=18 + 6 * (author_counts[freq_label] / max_x if max_x > 0 else 1),
            color=author_counts[freq_label],
            colorscale=[[0, '#B3D1F2'], [1, '#5567BB']],
            line=dict(width=1, color='#E0E0E0'),
            opacity=0.95, showscale=False,
        ),
        text=author_counts[freq_label],
        textposition='top center',
        textfont=dict(color='#5567BB', size=13),
        hovertemplate=(
            '<b>Author:</b> %{customdata}<br>'
            f'<b>{freq_label}:</b> %{{x}}<extra></extra>'
        ),
        customdata=author_counts['Authors'],
    ))

    for i in range(len(author_counts)):
        fig.add_shape(type='line', x0=0, x1=max_x, y0=i, y1=i,
                      line=dict(color='#E0E0E0', width=2), layer='below')

    tick_step = max(1, int(max_x // 5))
    x_ticks = list(range(0, int(max_x) + tick_step, tick_step))

    fig.update_yaxes(tickvals=list(range(len(author_counts))),
                     ticktext=author_counts['Authors'],
                     autorange='reversed', showgrid=False,
                     title='Authors', tickfont=dict(size=13))
    fig.update_xaxes(showgrid=True, gridcolor='#F0F0F0', zeroline=False,
                     tickvals=x_ticks, title=freq_label,
                     tickfont=dict(size=13), range=[0, max_x * 1.1])
    fig.update_layout(
        plot_bgcolor='white',
        font=dict(color='#222222', size=14, family='Segoe UI, Arial'),
        margin=dict(l=0, r=0, t=0, b=0),
        height=50 + 90 * len(author_counts),
        showlegend=False,
        hoverlabel=dict(bgcolor='white', font_size=13,
                        font_family='Segoe UI, Arial', bordercolor='#5567BB'),
        coloraxis_showscale=False,
    )
    
    
    return fig, table_relevant_authors
